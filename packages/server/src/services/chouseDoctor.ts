...
- system.parts (on-disk parts): database, table, partition, active, rows, bytes_on_disk, data_compressed_bytes, modification_time, level. GROUP BY partition to see partition sizes + part counts (scanning every partition = no pruning; many parts = merge pressure). STRICT GROUP BY (CH 24.11): every SELECT column MUST be in GROUP BY or wrapped in an aggregate — otherwise NOT_AN_AGGREGATE (code 215). When you GROUP BY partition with WHERE active, do NOT also put \`active\` in SELECT (it's already constrained to 1 by WHERE — drop it). Use \`SELECT partition, count() AS parts, sum(rows) AS rows, sum(bytes_on_disk) AS bytes\` — never \`SELECT partition, active, …\`.
...
- system.parts WHERE database = '…' AND table = '…' AND active : GROUP BY partition to see the active part COUNT + sizes per partition. Many small active parts (e.g. >300 in a partition) = merge pressure / too-frequent tiny inserts; hundreds/thousands of partitions = a partition key that's too fine. Example shape (CH 24.11 strict GROUP BY — do NOT put \`active\` in SELECT, it's already filtered): \`SELECT partition, count() AS parts, sum(rows) AS rows, formatReadableSize(sum(bytes_on_disk)) AS size FROM system.parts WHERE database = '…' AND table = '…' AND active GROUP BY partition ORDER BY partition DESC LIMIT 30\`.
